...
uint16_t SAMP::port = 7777;
uint32_t SAMP::token = 0;
uint16_t SAMP::cookies[2][256] = {0};
void (*SAMP::HandleQuery)(SOCKET instance, sockaddr_in client, uint8_t *data, size_t size) = nullptr;

void SAMP::Decrypt(uint8_t *dest, uint8_t *src, size_t length) {
    ...
    if (src[0] != checksum)
        std::cout << "[Warning] invalid checksum" << std::endl;
}

void SAMP::Encrypt(uint8_t *dest, uint8_t *src, size_t length) {
    static const uint8_t key[] = {
...
const char *SAMP::GenerateAuth(uint8_t *index) {
    srand(time(nullptr));
    *index = rand();
    return authTable[*index].send;
}

bool SAMP::CheckAuth(uint8_t index, const char *auth) {
    return strcmp(authTable[index].recv, auth) == 0;
}

const char *SAMP::GetAuth(const char *auth) {
    for(size_t index = 0; index < sizeof(authTable) / sizeof(authTable[0]); index++) {
        if(strcmp(authTable[index].send, auth) == 0)
            return authTable[index].recv;
    }

    return nullptr;
}

void SAMP::GenerateCookie() {
    for (size_t index = 0; index < 256; index++) {
        srand(time(nullptr));
        cookies[0][index] = rand() % (USHRT_MAX + 1);

        srand(time(nullptr));
        cookies[1][index] = rand() % (USHRT_MAX + 1);
    }
}

uint16_t SAMP::GetCookie(uint32_t address) {
    uint8_t *addressSplit = (uint8_t*)&address;

    return
        (cookies[0][addressSplit[0]] | cookies[1][addressSplit[3]] << 8) ^
        ((addressSplit[1] << 8) | addressSplit[2]);
}